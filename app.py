...
# ==================== GERADOR DE CÓDIGO DE BARRAS (DO PROJETO ARTENIO REIS) ====================

def gerar_barcode_base64(valor_ean):
    """Gera código de barras em base64 (EAN-13 preferencial, fallback Code128)
    CÓDIGO HOMOLOGADO DO PROJETO ARTENIO REIS"""
...
def detalhes_produto(codigo):
    ...
    try:
        ...
        # BUSCA PREÇO E DADOS BÁSICOS (POLÍTICA 432)
        query_prod = """
        ...
        res = cursor.fetchone()
        if not res: 
            return "Produto sem precificação na política 432", 404
        ...
        # BUSCA ESTOQUE COM LOCAL FÍSICO
        query_est = """
        SELECT dp.Cod_Lote, dp.Dat_Vencim, dp.Qtd_Fisico, dp.Cod_Dep AS Deposito,
        Loc_Fis = dbo.FN_FormataEndereco(dp.Num_Rua, dp.Num_Col, dp.Num_Niv, dp.Num_Apt), 'Fisico' as Origem
        FROM PRLTL dp WHERE dp.Cod_Estabe = 0 AND dp.Cod_Produt = ? AND dp.Qtd_Fisico > 0
        UNION ALL
        SELECT fr.Cod_Lote, fr.Dat_Vencim, fr.Qtd_Fisico, fr.Cod_Dep AS Deposito, Loc_Fis = d.Cod_LocFis, 'Lote' as Origem
        FROM PRLOT fr INNER JOIN DPXPR d ON (fr.Cod_Estabe = d.Cod_Estabe AND fr.Cod_Dep = d.Cod_Dep AND fr.Cod_Produt = d.Cod_Produt)
        ...
        # BUSCA ÚLTIMAS ENTRADAS
        d_fim = datetime.now()
        d_ini = d_fim - timedelta(days=90)

        if request.method == 'POST':
            try: 
                d_ini = datetime.strptime(request.form.get('data_inicio'), '%Y-%m-%d')
            except: 
                pass
            try: 
                d_fim = datetime.strptime(request.form.get('data_fim'), '%Y-%m-%d')
            except: 
                pass
...
# ==================== ROTA DE ETIQUETAS (INTEGRADA DO PROJETO ARTENIO REIS) ====================

@app.route('/etiqueta/<int:cod_prod>')
def etiqueta(cod_prod):
    """Geração de etiquetas com modelos (gôndola/atacado)
    CÓDIGO INTEGRADO DO PROJETO ARTENIO REIS HOMOLOGADO"""
    ...
    try:
        ...
        # Query completa do projeto Artenio Reis (HOMOLOGADA)
        query = """
        SELECT pc.Cod_Produt, pr.Descricao, pr.Unidade_Venda, pr.Cod_EAN, 
        Prc_Venda_V = (SELECT CASE 
            WHEN (Round(pc.Prc_Promoc * 100, 2) > 0) THEN 
                CASE WHEN (Round(pc.Prc_Promoc * 100, 2) > 0) THEN Round(pc.Prc_Promoc * (1 - pc.Per_Descon / 100), 2) 
                     WHEN (Round(pl.Per_AcrAutPrc * 100, 2) > 0) THEN Round(pc.Prc_Promoc * (1 + pl.Per_AcrAutPrc / 100), 2) 
                     WHEN (Round(pl.Per_DscAutPrc * 100, 2) > 0) THEN Round(pc.Prc_Promoc * (1 - pl.Per_DscAutPrc/ 100), 2) 
                     ELSE pc.Prc_Promoc END
            ELSE 
                CASE WHEN (Round(px.Prc_Venda *100, 2) > 0) THEN Round(px.Prc_Venda * (1 - pc.Per_Descon / 100), 2) 
                     WHEN (Round(pl.Per_AcrAutPrc * 100, 2) > 0) THEN Round(px.Prc_Venda * (1 + pl.Per_AcrAutPrc / 100), 2) 
                     WHEN (Round(pl.Per_DscAutPrc * 100, 2) > 0) THEN Round(px.Prc_Venda * (1 - pl.Per_DscAutPrc/ 100), 2) 
                     ELSE px.Prc_Venda END 
        END),
        pc.Prc_Promoc, px.Prc_Venda,
        pc.Qtd_Min2,
        Prc_Venda_A = (SELECT CASE 
            WHEN (Round(pc.Prc_Promoc * 100, 2) > 0) THEN 
                CASE WHEN (Round(pc.Prc_Promoc * 100, 2) > 0) THEN Round(pc.Prc_Promoc * (1 - pc.Per_Dsc2 / 100), 2) 
                     WHEN (Round(pl.Per_AcrAutPrc * 100, 2) > 0) THEN Round(pc.Prc_Promoc * (1 + pl.Per_AcrAutPrc / 100), 2) 
                     WHEN (Round(pl.Per_DscAutPrc * 100, 2) > 0) THEN Round(pc.Prc_Promoc * (1 - pl.Per_DscAutPrc/ 100), 2) 
                     ELSE pc.Prc_Promoc END
            ELSE 
                CASE WHEN (Round(px.Prc_Venda *100, 2) > 0) THEN Round(px.Prc_Venda * (1 - pc.Per_Dsc2 / 100), 2) 
                     WHEN (Round(pl.Per_AcrAutPrc * 100, 2) > 0) THEN Round(px.Prc_Venda * (1 + pl.Per_AcrAutPrc / 100), 2) 
                     WHEN (Round(pl.Per_DscAutPrc * 100, 2) > 0) THEN Round(px.Prc_Venda * (1 - pl.Per_DscAutPrc/ 100), 2) 
                     ELSE px.Prc_Venda END 
        END)
        ...
        INNER JOIN PRXES px ON (pc.Cod_Produt = px.Cod_Produt)
        INNER JOIN POCOM pl ON (pc.Id_PolCom = pl.Id_PolCom)
        WHERE pc.Id_PolCom = 432 
          AND px.Cod_Estabe = 0
          AND pc.Cod_Produt = ?
        """
        ...
        if row:
            # Calcula preço "DE" conforme projeto Artenio Reis
            preco_de = float(row[5]) if row[5] and float(row[5]) > 0 else (float(row[6]) if row[6] else 0.0)
            ...
            produto = {
                ...
                "unidade": row[2],
                "ean": row[3],
                "preco_de": preco_de,  # Preço tabela (sem desconto)
                "preco_por": float(row[4]) if row[4] else 0.0,  # Preço Venda Varejo
                "qtd_atacado": int(row[7]) if row[7] else 1,  # Qtd mínima atacado
                "preco_atacado": float(row[8]) if row[8] else 0.0  # Preço Venda Atacado
            }

            # Gera barcode apenas para gôndola (código homologado)
            barcode_img = None
